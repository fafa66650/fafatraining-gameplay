...
function prepareAudio(){
  Object.entries(audioFiles).forEach(([k,v])=>{
    if(!audioPool[k]){
      ...
      a.volume = k.startsWith('theme_') ? 0.55 : 1;
      audioPool[k] = a;
    }
  });
}
...
function startAmbience(){
  ...
  try {
    prepareAudio();
    ['theme_night','theme_jungle','theme_lab'].forEach(k=>{ if(audioPool[k]){audioPool[k].pause();audioPool[k].currentTime=0;}});
    const track = state.theme==='jungle'?'theme_jungle':state.theme==='lab'?'theme_lab':'theme_night';
    audioPool[track].play().catch(()=>{});
  } catch(e) {}
}
  ...
  APP.innerHTML = `
    <div class="hero">
      ...
      <div>
        ...
        <h1>GAME ARENA</h1>
        <div class="subtitle">Une plateforme d’escape game immersive pour créer des missions interactives, répartir les équipes, gérer la difficulté et lancer des expériences cinématiques sur mobile comme ordinateur.</div>
...
function startTicking(){
  ...
  const tick = () => {
    ...
    if(!state.timerActive) return;
    state.time = Math.max(0, Number(state.time) - 1); const timerEl=document.getElementById('live-timer'); if(timerEl){timerEl.textContent='⏱ '+fmt(state.time);}
  ...
  const main = `
    <section class="game-shell fadein">
      <div class="hud">
        ...
        <div class="hud-right"><div id="live-timer" class="badge timer ${timerColor()}">⏱ ${fmt(state.time)}</div></div>
      </div>
...
function renderFinal(showAdmin){
  ...
  const main = `<section class="game-shell fadein">
      <div class="hud"><div class="hud-left"><div class="badge points">Final de mission</div><div class="badge meta">Fragments : ${state.fragments.length}</div></div><div class="hud-right"><div id="live-timer" class="badge timer ${timerColor()}">⏱ ${fmt(state.time)}</div></div></div>
